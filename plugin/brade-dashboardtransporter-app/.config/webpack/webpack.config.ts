...
const config = async (env: Env): Promise<Configuration> => {
  // 🔧 getEntries() está vindo vazio no seu projeto -> não gera module.js
  // Então: tenta entries do scaffold, e se vier vazio, força entry em src/module.tsx
  const scaffoldEntries = await getEntries();
  const entries =
    scaffoldEntries && Object.keys(scaffoldEntries).length > 0
      ? scaffoldEntries
      : {
          module: path.resolve(process.cwd(), SOURCE_DIR, 'module.tsx'),
        };
